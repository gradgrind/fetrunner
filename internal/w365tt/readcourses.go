...
func (dbi *W365TopLevel) readSubjects() {
	...
	sloop:
		...
		if nok {
			base.LogError("Subject Tag (Shortcut) defined twice: %s",
				e.Tag)
			e.Tag += "$"
			goto sloop
		}
...
func (dbi *W365TopLevel) readSuperCourses() {
	...
	for _, spc := range dbi.SuperCourses {
		...
		if !ok {
			base.LogError("Unknown EpochPlan in SuperCourse %s:\n  %s",
...
func (dbi *W365TopLevel) getCourseSubject(
	...
	if len(srefs) == 1 {
		...
		base.LogError("Course/SubCourse has no subject: %s", courseId)
		// Use a dummy Subject.
		var ok bool
	...
	if len(rrefs) > 1 {
		...
		if ok {
			...
			if ok {
				...
				base.LogError("Invalid room in Course/SubCourse %s:\n  %s",
					courseId, rref0)
			}
		}
	}
...
func (dbi *W365TopLevel) getCourseGroups(
	...
	for _, gref := range grefs {
		...
		if !ok {
			base.LogError("Invalid group in Course/SubCourse %s:\n  %s",
				courseId, gref)
			continue
		}
...
func (dbi *W365TopLevel) getCourseTeachers(
	...
	for _, tref := range trefs {
		...
		if !ok {
			base.LogError("Unknown teacher in Course %s:\n  %s",
				courseId, tref)
		}
