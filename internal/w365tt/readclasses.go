...
func (dbi *W365TopLevel) readClasses() {
    ...
    dloop:
        for i, wdiv := range e.Divisions {
            ...
            for _, g := range wdiv.Groups {
                ...
                if ok {
                    if flag {
                        base.LogError("--W365_GROUP_IN_MULTIPLE_DIVISIONS %s", g)
                        continue dloop
                    }
                    ...
                    base.LogError("--W365_UNKNOWN_GROUP Class %s, Division %s, Group: %s",
                        e.Tag, wdiv.Name, g)
                }
            }
            ...
            if len(glist) < 2 {
                base.LogWarning(
                    "DIVISION_HAS_TOO_FEW_GROUPS Class: %s, Division: %s",
                    e.Tag, wdiv.Name)
            }
    ...
    for _, n := range dbi.Groups {
        if pregroups[n.Id] {
            ...
            base.LogError("--W365_GROUP_NOT_IN_DIVISION Removing %s", n.Id)
        }
    }
}
