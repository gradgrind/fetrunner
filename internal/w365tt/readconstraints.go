...
func a2r(r any) NodeRef {
    ...
    if r != nil {
        base.LogError("--W365_INVALID_NodeRef_IN_CONSTRAINT %+v", r)
    }
    return ""
}
...
func a2i(i any) int {
    ii, ok := i.(float64)
    if !ok {
        base.LogError("--W365_INVALID_NUMBER_IN_CONSTRAINT %+v", i)
        return 0
    }
    return int(ii)
}
...
func a2rr(rr any) []NodeRef {
    ...
    if ok {
        ...
        base.LogError("--W365_INVALID_NodeRef_LIST_IN_CONSTRAINT %+v", rr)
    }
    return rlist
}
...
func a2ii(ii any) []int {
    ...
    if ok {
        ...
        base.LogError("--W365_INVALID_NUMBER_LIST_IN_CONSTRAINT %+v", ii)
    }
    return ilist
}
    ...
    for _, e := range db0.Constraints {
        ...
        case base.C_AutomaticDifferentDays:
            if automatic_different_days {
                base.LogError("--W365_MULTIPLE_CONSTRAINTS_OF_TYPE %s", base.C_AutomaticDifferentDays)
            } else {
        ...
        case base.C_DoubleActivityNotOverBreaks:
            if double_activity_not_over_breaks {
                base.LogError("--W365_MULTIPLE_CONSTRAINTS_OF_TYPE %s", base.C_DoubleActivityNotOverBreaks)
            } else {
        ...
        case base.C_ParallelCourses:
            ndb.NewParallelCourses(
                ...
                a2rr(e["Courses"]))
        default:
            base.LogError("--W365_CONSTRAINT_INVALID: %s", cw365)
        }
    }
}
