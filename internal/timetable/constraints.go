    ...
    for _, c := range constraint_map[base.C_ParallelCourses] {
        ...
        for i, cref := range courses {
            ...
            if i == 0 {
                ...
                base.LogError(
                    "--PARALLEL_COURSES_HAVE_DIFFERENT_ACTIVITIES %s",
                    strings.Join(clist, ","))
                continue cloop
            }
            for j, ai := range cinfo.Activities {
                ...
                if i == 0 {
                    ...
                    base.LogError(
                        "--PARALLEL_COURSES_HAVE_ACTIVITY_MISMATCH %s",
                        strings.Join(clist, ","))
                    continue cloop
                }
...
func (tt_data *TtData) days_between_activities(
    ...
    if len(unfixeds) == 0 || (len(fixeds) == 0 && len(unfixeds) == 1) {
        ...
        //TODO?
        base.LogWarning(
            "--SUPERFLUOUS_DaysBetween_CONSTRAINT_ON_COURSE %s",
            tt_data.View(cinfo))
        return allist
    }
    ...
    if weight != 0 || consecutiveIfSameDay {
        ...
        for _, alist := range aidlists {
            if len(alist) > tt_data.ndays {
                //TODO?
                base.LogWarning(
                    "--TOO_MANY_DifferentDays_CONSTRAINTS Course %s",
                    tt_data.View(cinfo))
                continue
            }
