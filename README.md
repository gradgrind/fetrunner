...
The `fet-cl.exe` built when `FET` is built normally is compiled as a console application. This means that whenever it is run by `fetrunner`, a new console will be popped up. This makes a real mess! Thus a custom build of `fet-cl` is required. I have given the executable a new name, `fet-clw.exe` to distinguish it; it can be generated by copying `src/src-cl` to `src/src-clw` and removing `cmdline` from CONFIG in `src/src-clw.pro`, adding `src/src-clw.pro` to SUBDIRS in `fet.pro` and then compiling. If `fet` and `fet-cl` don't need to be compiled, `src/src.pro` and `src/src-cl.pro` can be removed from SUBDIRS in `fet.pro`.

### Building the GUI together with `FET`

It may be more convenient to build `fetrunner` together with `FET`, especially on Windows, where a custom build of `fet-cl` is required anyway. By adding a `qmake` build file to `fetrunner`, this can be achieved in a fairly straightforward way, see `gui/README.md`.
