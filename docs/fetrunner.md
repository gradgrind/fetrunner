...
**There is also a "Ref" field**. This is used when the original FET file is generated by some other software. This field then references the object in that other software. In order to make these references available to `fetrunner`, the "Comments" fields of the various FET nodes are used.
