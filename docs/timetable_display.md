...
An approach which avoids the multiple-tiles-per-slot problem completely would be to base the display on an atomic group rather than a class. This would be a view for the set of students belonging to the displayed atomic group. Such a group view could be designated by the set of used (by activities) groups containing this atomic group. Although this would mean rather a lot of views for classes with many atomic groups, it seems to be the most straightforward way of handling the problem. It may be desirable to have a two-tier menu (selecting first class and then atomic group) rather than presenting a long list of combined class-group entries.
