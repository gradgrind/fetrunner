...
func a2r(logger base.FileLogger, r any) NodeRef {
	rr, ok := r.(string)
	if ok {
		return NodeRef(rr)
	}
...
func a2i(logger base.FileLogger, i any) int {
	ii, ok := i.(float64)
	if !ok {
...
func a2rr(logger base.FileLogger, rr any) []NodeRef {
	rlist := []NodeRef{}
	rrr, ok := rr.([]any)
	if ok {
		for _, r := range rrr {
...
func a2ii(logger base.FileLogger, ii any) []int {
	ilist := []int{}
	iii, ok := ii.([]any)
	if ok {
		for _, i := range iii {
